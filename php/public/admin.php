...
<body>
  <header>
    ...
    <div class="toolbar">
      <label>
        Admin-token
        <input id="adminTokenInput" type="password" autocomplete="off" placeholder="Alleen nodig op publieke server">
      </label>
  ...
  <script>
    ...
    tokenInput.value = sessionStorage.getItem("teletyptelAdminToken") || "";
    refreshButton.addEventListener("click", loadAdmin);
    tokenInput.addEventListener("change", () => {
      sessionStorage.setItem("teletyptelAdminToken", tokenInput.value);
      loadAdmin();
    });

    async function adminFetch(url, options = {}) {
      const headers = { ...(options.headers || {}) };
      if (tokenInput.value) headers["X-TeleTypTel-Admin-Token"] = tokenInput.value;
