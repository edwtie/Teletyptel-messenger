...
function writeRuntimeScripts(string $runtimePath, string $rootPath, array $state): array
{
    ...
    $mamSnippet = "# TeleTypTel ejabberd MAM module fragment\n"
        ...
        . "# For Docker/official ejabberd images the config is often /opt/ejabberd/conf/ejabberd.yml.\n"
        . "# `db_type: sql` requires ejabberd SQL storage to be configured and reachable.\n"
        . "  mod_mam:\n"
        . "    db_type: sql\n"
        . "    default: always\n"
    ...
    $ejabberdMam = "#!/usr/bin/env sh\n"
        ...
        . "if [ -z \"\$CONFIG\" ]; then\n"
        . "  for candidate in /etc/ejabberd/ejabberd.yml /opt/ejabberd/conf/ejabberd.yml; do\n"
        . "    if [ -f \"\$candidate\" ]; then\n"
        . "      CONFIG=\"\$candidate\"\n"
        . "      break\n"
        . "    fi\n"
        ...
        . "fi\n"
        . "if [ ! -f \"\$CONFIG\" ]; then\n"
        . "  echo \"ejabberd config not found. Set EJABBERD_CONFIG=/path/to/ejabberd.yml\"\n"
        . "  exit 1\n"
        . "fi\n"
        . "DB_TYPE=\${EJABBERD_MAM_DB_TYPE:-auto}\n"
        . "if [ \"\$DB_TYPE\" = \"auto\" ]; then\n"
        . "  if grep -Eq '^[[:space:]]*(default_db:[[:space:]]*sql|sql_type:|sql_server:)' \"\$CONFIG\"; then\n"
        . "    DB_TYPE=sql\n"
        . "  else\n"
        . "    DB_TYPE=mnesia\n"
        . "  fi\n"
        . "fi\n"
        . "if [ \"\$DB_TYPE\" = \"sql\" ]; then\n"
        . "  SQL_HOST=\${EJABBERD_SQL_HOST:-}\n"
        . "  SQL_PORT=\${EJABBERD_SQL_PORT:-}\n"
        . "  if [ -z \"\$SQL_HOST\" ]; then SQL_HOST=\$(awk '/^[[:space:]]*sql_server:/ { gsub(/\"/, \"\", \$2); print \$2; exit }' \"\$CONFIG\"); fi\n"
        . "  if [ -z \"\$SQL_PORT\" ]; then SQL_PORT=\$(awk '/^[[:space:]]*sql_port:/ { print \$2; exit }' \"\$CONFIG\"); fi\n"
        . "  if [ -n \"\$SQL_HOST\" ] && [ -n \"\$SQL_PORT\" ] && command -v nc >/dev/null 2>&1 && ! nc -z \"\$SQL_HOST\" \"\$SQL_PORT\"; then\n"
        . "    echo \"Warning: SQL endpoint \$SQL_HOST:\$SQL_PORT is not reachable. ejabberd SQL-backed MAM/offline storage can fail until MySQL/MariaDB is running.\"\n"
        . "  fi\n"
        . "fi\n"
        ...
        . "fi\n"
        . "if command -v ejabberdctl >/dev/null 2>&1; then sudo ejabberdctl status || true; fi\n";
